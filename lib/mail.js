...
// Default professor email address (if configured).
export function getProfessorEmail() {
  return process.env.PROFESSOR_EMAIL || process.env.ABC_PROFESSOR_EMAIL || '';
}

// Base URL used to generate verification links.
export function getAppBaseUrl() {
  return process.env.APP_BASE_URL || 'http://localhost:3000';
}

// Send the verification email to a professor for course review.
export async function sendProfessorVerificationEmail({
  ...
  // Configure the transport once with the provided SMTP credentials.
